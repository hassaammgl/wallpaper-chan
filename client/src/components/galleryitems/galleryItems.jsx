    ...
    >
      ...
      {item.title && (
        <div className="absolute bottom-0 left-0 right-0 translate-y-2 p-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          <p className="truncate text-sm font-medium text-white drop-shadow-lg">{item.title}</p>
          <div className="mt-1 flex gap-1">
            {item.deviceType && (
              <span className="rounded bg-black/40 px-1.5 py-0.5 text-[10px] capitalize text-white/80">{item.deviceType}</span>
            )}
            {item.resolution && (
              <span className="rounded bg-black/40 px-1.5 py-0.5 text-[10px] text-white/80">{item.resolution}</span>
            )}
          </div>
        </div>
      )}
